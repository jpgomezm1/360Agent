...
class RAGService {
constructor() {
 // Cliente Weaviate - CONFIGURACIÓN CORREGIDA basada en el test exitoso
 this.weaviateClient = weaviate.client({
   scheme: 'https',
   host: process.env.WEAVIATE_URL, // Usar directamente la variable de entorno
...
/**
* Obtener estadísticas de la base de conocimiento - VERSIÓN CORREGIDA
* @returns {Promise<Object>} Estadísticas
*/
async getKnowledgeStats() {
 try {
   // ESTRATEGIA 1: Usar GET query con count
   const response = await this.weaviateClient.graphql
     .get()
     .withClassName(this.className)
   ...
   // Si hay datos, hacer una búsqueda más amplia para contar
   if (response.data?.Get?.[this.className]) {
     // Hacer una query más amplia para obtener count aproximado
     const countResponse = await this.weaviateClient.graphql
       .get()
       .withClassName(this.className)
       .withFields('_additional { id }')
       .withLimit(1000) // Límite alto para contar
       .do();

     const count = countResponse.data?.Get?.[this.className]?.length || 0;
     
     return {
       totalDocuments: count >= 1000 ? '1000+' : count,
       className: this.className,
       status: 'active'
     };
   }
   ...
   // FALLBACK: Verificar que la clase existe usando schema
   try {
     const schema = await this.weaviateClient.schema.getter().do();
     const classExists = schema.classes?.find(c => c.class === this.className);
     
     return {
       totalDocuments: classExists ? 'unknown' : 0,
       className: this.className,
       status: classExists ? 'schema-only' : 'no-class'
     };
   } catch (fallbackError) {
     // Último fallback: reportar error pero no fallar
     return {
